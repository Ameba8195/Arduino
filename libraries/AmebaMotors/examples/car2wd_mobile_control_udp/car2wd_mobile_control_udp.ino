/*
 * This sketch use control car via udp connection
 *
 * This sketch bring up wifi in AP mode.
 * User can change mobile phone's wifi AP with correspond ssid.
 * Device open UDP socket on port 5001, and app connect to
 * it on default AP IP 192.168.1.1
 * App send raw data when control data is changed.
 ...
 * 
 * This sketch can use with sketch "car2wd_mpu6050_control".
 **/
...
void loop() {
  ...
  if (len > 0) {
    ...
    Serial.println("recv timeout, stop car");
    memset(buffer, 0, 256);
    handleData((const char *)buffer);
  }
}
